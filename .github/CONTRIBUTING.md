...
The following content is a serial of guidelines for contributing to `Pugio.js`. Before starting your work, you are supposed to read the following advice:
...
4. Merge and tag a new version in `pugio-commons@<number>.<number>.<number>` format
...
> NOTE: Only core contributors can publish packages, by merging from other branch to `master` and add a tag in `pugio@<number>.<number>.<number>` format. After a commit on `master` is beeing tagged, the CI will build and publish all packages automatically.
